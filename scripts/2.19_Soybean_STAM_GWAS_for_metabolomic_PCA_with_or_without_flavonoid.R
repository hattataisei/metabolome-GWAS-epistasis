##########################################################################################
######  Title: 2.19_Soybean_STAM_GWAS_for_metabolomic_PCA_with_or_without_flavonoid ######
######  Author: Taisei Hatta (hatta@ut-biomet.org)                                  ######
######  Affiliation: Lab. of Biometry and Bioinformatics, The University of Tokyo   ######
######  Date: 2024/04/04 (Created), 2024/08/16 (Last Updated)                       ######
##########################################################################################
...
dirMidSTAMGWASPCAFlavonoid <- paste0(dirMidSTAMBasePCAFlavonoid, scriptID,
                            "_GWAS_for_PCA_flavonoid/")
dir.create(dirMidSTAMGWASPCAFlavonoid)
...
##### 2.2. Read genotypic values of Metabolomic data for PCA into R #####
#### 2.2.1. in 2017 ####
# targetTypeNo <- 1
cultivationInfo <- "2017"
targetInfo <- "Metabolome"
...
for (targetTypeNo in 1:length(targetType)) {
  ...
  gvMetab <- read.csv(paste0("midstream/2.18_BSH_for_PCA_with_or_without_flavonoid/2.18_lmer_genotypic_values_", "for_PC_Score_", "for_", targetTypeNow, "_metab_in_", cultivationInfo, ".csv"),                                           row.names = 1)
  ...
  dirCultivationNow <- paste0(dirMidSTAMGWASPCAFlavonoid, scriptID,
                              "_", cultivationInfo, "_",
                              targetInfo, "_results/")
